...
describe("schedule/trigger: POST /trigger", () => {
  ...
  it("every call is a call, and the reply says WHERE TO LOOK", async () => {
    // No occurrence and no dedup: nothing here can tell one call from another, and retry policy is the
    // caller's because only the caller knows whether its work tolerates running twice.
    //
    // AN IDEMPOTENCY KEY WAS BUILT HERE AND REMOVED. It deduplicated the CALL, not the WORK: a turn
    // that sent one message and then died would answer a keyed retry with "already ran" — safety
    // exactly where it is absent. What the reply carries instead is the session, which is where that
    // turn's output actually is.
    const { agent, calls } = recordingAgent();
    const { handle } = await handlerFor([hourly()], agent);
    for (let i = 0; i < 3; i++) {
  ...
  it("a call arriving while the previous turn holds the session is reported, not run", async () => {
    // A declared unit of work has ONE session, so this is the route's only "did not run" — and it is
    // not a failure. Reported as itself, with what the caller needs to decide (try later), rather than
    // as an error it would retry blindly.
    const { agent, calls } = recordingAgent([
    ...
    expect(body.reason).toContain("still running");
    expect(body.failed).toBeUndefined(); // busy is a policy outcome, not a fault
    expect(calls).toHaveLength(1); // the turn was attempted; the session refused it
  });
