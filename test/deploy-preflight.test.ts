...
describe("deploy/preflight: the host-neutral pre-flight", () => {
  ...
  it("http.host only decides the bind when the Dockerfile does not", async () => {
    ...
    // A GENERATED Dockerfile with no --bind is the case where config still decides, and where the
    // finding is certain enough to gate.
    ...
    // A HAND-WRITTEN one is never gated on config, whatever config says: its CMD may pass --bind
    // 0.0.0.0, which beats http.host, and reading that is not something this does.
    await writeFile(join(dir, "Dockerfile"), 'FROM node:22-slim\nCMD ["fastagent", "start", "/app"]\n');
    const handWritten = await call(dir, cfg("127.0.0.1"), { run: true });
    expect(handWritten.ok).toBe(true);
    if (handWritten.ok) expect(handWritten.messages.some((m) => /not verified/.test(m.text))).toBe(true);
