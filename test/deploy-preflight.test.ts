...
describe("deploy pre-flight: a kept Dockerfile must still bind the wildcard", () => {
  it("gates a stale GENERATED file; asks about a hand-written one without claiming to have checked", async () => {
    ...
    // the container logs says why. `writeArtifacts` keeps an existing file, so nothing else notices.
    const dir = await workspace();
    const cfg: FastagentConfig = { model: "openai/gpt-4o-mini" };
    const stale = '# Generated by `fastagent deploy`\nFROM node:22-slim\nCMD ["fastagent", "start", "/app"]\n';
...
describe("deploy pre-flight: a generated marker is not a promise about the CMD", () => {
  it("reads the CMD line, not the file, and never lets http.host vouch for a hand-written one", async () => {
    const dir = await workspace();
    const cfg: FastagentConfig = { model: "openai/gpt-4o-mini" };

    // A `--bind 0.0.0.0` in a COMMENT does not bind anything. Scanning the file would clear this;
    // reading the CMD we wrote does not.
    await writeFile(
      join(dir, "Dockerfile"),
      '# Generated by `fastagent deploy`\n# was: --bind 0.0.0.0\nFROM node:22-slim\nCMD ["fastagent", "start", "/app"]\n',
    );
    const commented = await call(dir, cfg, { run: true });
    expect(commented.ok).toBe(false);

    // And a hand-written CMD is free to pass `--bind 127.0.0.1`, which beats http.host — so a
    // wildcard in config cannot vouch for it. The prompt stays.
    await writeFile(join(dir, "Dockerfile"), 'FROM node:22-slim\nCMD ["fastagent", "start", "/app", "--bind", "127.0.0.1"]\n');
    const overridden = await call(dir, { ...cfg, http: { host: "0.0.0.0" } }, { run: true });
    expect(overridden.ok).toBe(true);
    if (overridden.ok) expect(overridden.messages.some((m) => /not verified/.test(m.text))).toBe(true);
  });
});
