...
| `agentcore` | CloudFormation ACCEPTING the YAML this repo emits by hand, forwarder and schedule branches included (read-only, free, and the only check that the template parses). Also what a missing stack SAYS, in both directions through `aws-cli.ts`'s `ABSENT` — the wording that decides whether a deploy warns it is about to replace the agent's memory |
