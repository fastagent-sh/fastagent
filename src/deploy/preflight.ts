...
export async function preflightDeploy(input: {
    ...
    messages.push({
      ...
      text: hasOtherLock
        ? `the generated Dockerfile is npm-based — your pnpm/yarn lockfile is NOT used (build runs ` +
          `\`npm install\`, not reproducible). Vendor a package-lock.json, or take ownership of the Dockerfile ` +
          `(delete its "Generated by" marker line, else every deploy regenerates it) and adapt it to your package manager.`
  ...
  const existingDockerfile =
    (config.deploy?.apt?.length || model.envValue) && (await exists(dockerfileHome))
      ? await readFile(dockerfileHome, "utf8")
      : undefined;
  const handWritten =
    existingDockerfile !== undefined && !isGeneratedDockerfile(existingDockerfile) ? existingDockerfile : undefined;
  if (handWritten !== undefined) {
    ...
    // reported — a known crash-loop when nothing else names one, and a silent mismatch when the config does. The
    // INSTRUCTION is the question: a `# FASTAGENT_MODEL comes from compose` comment must not open the gate.
    const modelDropped = model.envValue !== undefined && !/^\s*ENV\s+FASTAGENT_MODEL[=\s]/m.test(handWritten);
    const dropped = [
...
function resolveDeployModel(
  ...
  // It is about to be generated INTO a Dockerfile, where whitespace truncates the instruction, a trailing `\`
  // swallows the next line, and `$`/quotes/backticks are build-time syntax. So the check bans exactly those and
  // asks for the one shape a spec always has, a `provider/` prefix — a modelId may itself contain `/` (795 of pi's
  // 1354 built-in specs do, e.g. `baseten/zai-org/GLM-5.3`) and `~` (openrouter's aliases), and resolveModel only
  // ever splits on the FIRST slash.
  if (fromEnv && !/^[^\s\\$"'`]+\/[^\s\\$"'`]+$/.test(fromEnv)) {
    throw new Error(
      `FASTAGENT_MODEL in ${valueFile} is not a usable "provider/modelId" spec — it is baked into a Dockerfile ` +
        `ENV, so whitespace, quotes, backslashes, \`$\` and backticks cannot appear in it: ${JSON.stringify(fromEnv)}`,
    );
  }
