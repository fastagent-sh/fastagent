...
function flyToml(appName: string, port: number, hasGithub: boolean): string {
  // GitHub turns are fire-and-forget with no replay (unlike Telegram's L1 turn store), so scale-to-zero
  // could drop an in-flight review. Definition-aware default: keep one machine running when a github
  // channel is present (it still suspends the extra capacity; it just never takes the LAST one down).
  const min = hasGithub
...
export function planFlyDeploy(input: FlyPlanInput): FlyPlan {
  ...
  const artifacts: Artifact[] = [
    { path: "fly.toml", content: flyToml(appName, port, channels.includes("github")) },
