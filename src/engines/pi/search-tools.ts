/** The built-in `search_tools` loader — the discovery surface for deferred tools (defineTool `deferred: true`). */
import { z } from "zod";
...
export function withSearchTool(tools: MountedTool[]): MountedTool[] {
  ...
  if (isDeferredTool(fixed)) {
    // A deferred LOADER is a contradiction — it is the only entry point to the deferred tools, so nothing could ever
    // activate it (or, through it, them).
    log.warn(
...
/** Miss-path listing cap — same rationale as the activation cap. */
  ...
  return defineTool({
    ...
    async execute(input, ctx) {
      ...
      const tokens = input.query
        ...
        .split(/[^a-z0-9]+/)
        .filter(Boolean);
      // AND semantics: EVERY token must hit — "adding a word narrows" must actually hold, or the over-cap "narrow the
      // query" instruction sends the model in circles (OR would widen with each word, and a shared prefix like
      // "fetch" could make a whole tool family permanently over-cap).
      ...
      // A query with no searchable tokens (all punctuation/symbols) must not match.
      if (tokens.length === 0)
        return `"${input.query}" contains no searchable keywords — describe the capability you need.`;
      // Exact-name shortcut: the guaranteed escape hatch from the cap — a query that IS an INACTIVE registered tool's
      // name addresses that one tool, no keyword scoring in the way.
      ...
      // Same listing cap as every other branch — a wide token can match most of the ACTIVE set too (in chat that
      // includes pi's default tools), and no answer may pour a catalog into the context.
      ...
      if (inactiveMatches.length === 0) {
        ...
        // Cap the miss listing like the activation cap — both guard the same semantic (don't pour the catalog back
        // into the context the deferral exists to protect).
      ...
      if (inactiveMatches.length > MAX_ACTIVATIONS_PER_SEARCH) {
        // Same listing cap as the miss path — an over-cap answer must not pour the catalog into the context either.
      ...
      // Report what actually happened, not what was attempted: a parallel sibling call may have activated the same
      // matches first, leaving nothing new here.
