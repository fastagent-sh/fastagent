...
export async function writeArtifacts(
  ...
  for (const a of artifacts) {
    ...
    // Regeneration is not silent: an operator who edited a file deploy owns has to see it replaced, and be told
    // the one way to keep such edits.
    if (existing !== undefined && regenerate) {
      console.error(
        `[fastagent] refreshed ${a.path} — it is generated, so any local edit was replaced ` +
          `(delete its "Generated by" marker line to take ownership; deploy then keeps your file).`,
      );
    }
