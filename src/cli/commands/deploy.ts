  ...
  await target.deploy({
    ...
    write: async (artifacts, options) => {
      ...
      if (opts.run && stale.length > 0) {
        failStartup(
          new Error(
            `deploy stopped: ${stale.join(", ")} no longer match what this definition generates, and --run would ` +
              `deploy from them. Re-run with --force to regenerate, or delete each file's "Generated by" marker ` +
              `line to own it yourself.`,
          ),
        );
      }
    },
