...
const CHANNEL_SCAFFOLDS: Record<ChannelKind, ChannelScaffold> = {
  ...
  feishu: {
    ...
    steps: [
      "before publishing: approve the sensitive im:message.group_msg permission for context-aware groups (the CLI adds it to the app draft when supported); it delivers all group messages so bare replies in the agent's own threads can invoke and other unsummoned discussion can buffer. A bare reply additionally needs a message-read scope (e.g. im:message:readonly): the channel lists a thread's recent messages to see who is taking part in it",
  ...
  lark: {
    ...
    steps: [
      ...
      "before publishing: approve the sensitive im:message.group_msg permission for context-aware groups (add it manually if Lark's config API fallback was used); it delivers all group messages so bare replies in the agent's own threads can invoke and other unsummoned discussion can buffer. A bare reply additionally needs a message-read scope (e.g. im:message:readonly): the channel lists a thread's recent messages to see who is taking part in it",
