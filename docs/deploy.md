...
Resident recipes require **one active replica** with durable storage. Multiple replicas need shared storage and coordination for sessions, channel state, and scheduled work; separate volumes split those records. A second process opening the same state directory through `dev`/`start`/`invoke`/`fire` (or `createAgentService`) is refused at startup, naming the holder's pid. (`chat` is not in that guard: pi's TUI keeps its own session storage.) A container killed outright (OOM, `docker kill`, a platform migration) leaves its claim behind for up to 15 seconds. A booting server waits that out rather than failing (a one-shot `invoke`/`fire` still refuses quickly, so a live writer is not a hang), and a refusal that does happen says whether the holder is still running. The `PiSessionRecordStore` / `Lease` seams cover engine sessions (see [Embedding](embedding.md)), not every channel's state.
