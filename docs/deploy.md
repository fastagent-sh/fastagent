...
Resident recipes require **one active replica** with durable storage. Multiple replicas need shared storage and coordination for sessions, channel state, and scheduled work; separate volumes split those records. A second process opening the same state directory through `dev`/`start`/`invoke`/`fire` (or `createAgentService`) is refused at startup, with the holder's own pid and command line. (`chat` is not in that guard: pi's TUI keeps its own session storage.) The claim is a listening socket, so a holder killed outright — OOM, `docker kill`, a platform migration — leaves nothing behind: the next process takes the claim immediately, with no window to wait out. That guard is per machine (per container, in a container); one process per mounted volume is the deployment lease's job, not this one's. The `PiSessionRecordStore` / `Lease` seams cover engine sessions (see [Embedding](embedding.md)), not every channel's state.
