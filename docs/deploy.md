...
- **Definition edits survive restarts.** Markdown and the TypeScript in `tools/` are live-read each turn, and the TypeScript in `routines/` within 30 seconds (on AgentCore, routines change with a deploy: its clock is the EventBridge rules); channels, configuration and any other file under `tools/` or `routines/` need a service restart. A new release can replace those edits. The deployed system prompt explains this boundary.
