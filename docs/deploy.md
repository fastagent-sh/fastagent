...
## Serving an existing directory (standalone layout)

When the workspace is standalone (the whole workspace lives in `./.fastagent/` and the parent directory is the workbench — see [Configuration](configuration.md)), `deploy` uses the SAME semantic as flat — **bake the workbench as the image, WYSIWYG** (what you see is what ships: git or not, clean or dirty) — with the artifacts namespaced:

- **Artifacts land at the workspace root** — `.fastagent/Dockerfile`, `.fastagent/Dockerfile.dockerignore`, and `.fastagent/fastagent.compose.yml` / `.fastagent/fly.toml` / `.fastagent/railway.json` — so they never collide with the host repo's own Docker/deploy files. **One host-tree write**: a `.dockerignore` at the workbench root (context-packers only read that form; it carries the machinery excludes `**/.secrets`, `**/.state`, `**/.cache`, plus `**/node_modules` and `**/.env*`, and does *not* exclude `.git`). If the host already has one it is **kept — even under `--force`** (it's the host's file), and preflight warns specifically when it lacks the secret/machinery excludes (the packer would bake secrets into the image) or `**/node_modules`, and notes a `.git` exclude (kills the agent's pull/push loop). Docker Compose builds from the workbench root through the namespaced file; the Fly runbook passes explicit flags (`fly deploy . --config .fastagent/fly.toml --dockerfile .fastagent/Dockerfile`); on Railway, point the service at `.fastagent/railway.json` (Settings → Config-as-code — dashboard-only).
