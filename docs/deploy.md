...
- **EventBridge Scheduler rules** firing each `schedules/*.ts` cron (the container arms no resident timers; the rule names the schedule and the container names the occurrence, so redelivery is idempotent). A cron EventBridge cannot express is refused at deploy time, never silently dropped;
