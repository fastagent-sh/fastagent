...
| `logs agentcore [dir]` | Discover the deployed stack's CloudWatch log group and run `aws logs tail`. Defaults to the Runtime's `[runtime-logs]` streams (application stdout/stderr only, excluding OTEL/spans); `--source forwarder` selects the separate Lambda ingress logs. `--since <duration>` sets the history window and `--follow` keeps polling. Read-only; does not change `FASTAGENT_LOG_LEVEL`. |
