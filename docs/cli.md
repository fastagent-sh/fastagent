...
| `deploy k8s [dir]` | Generate Kubernetes manifests (`k8s/`: namespace, PVC at `/data` (`ReadWriteOncePod`), single-replica `Recreate` Deployment with `/health` probes, Service, and — with `--host <domain>` — an Ingress) plus the shared `Dockerfile`/`.dockerignore`, and print a kubectl runbook: build + push the image (`--image <ref>`; the cluster only pulls), create the namespace + secret, `kubectl apply -k k8s/`, wait for the rollout, register webhooks. `--run` applies against the current kubectl context (assumes the image is pushed; carries your local credential; needs kubectl). `--force` overwrites artifacts. |
