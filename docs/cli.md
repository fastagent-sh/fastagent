...
| `destroy agentcore [dir] [--run]` | Delete every AWS resource the deploy created, in the order the dependencies demand. Without `--run` it deletes nothing and lists what is out there. AgentCore-only: the other hosts have their own (`fly apps destroy`, `railway down`, `docker compose down -v`). |
