...
**fastagent sets the mode of the files it creates, and nothing else.** A credential file it creates
(`auth.json`, or a `.secrets/.env` it makes itself) is `0600`. A file it did not create keeps the mode
its owner gave it — a `.env` you produced with `cp .secrets/.env.example .secrets/.env` carries that
command's mode, and `chmod 600` on it is yours to run. Directory permissions are yours too: fastagent
never sets or repairs them. Rails draws the same line (it chmods the `master.key` it generates and
leaves `config/` alone), as does the aws CLI (a `0600` `~/.aws/config` inside a `0755` `~/.aws`).
