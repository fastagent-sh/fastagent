...
`dev` assembles the agent and serves it on `:8787`. persona.md/AGENTS.md/`skills/`/`tools/` edits go live on the next turn; other code edits (`channels/`, `routines/`, config) restart the worker. The data plane is `POST /invoke`, always served.
