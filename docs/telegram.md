...
`files/` is scratch, not storage: **it is emptied when the channel mounts** (every `start`, and every `dev` restart), so a path from a previous process reads ENOENT. That is the `/tmp` contract, and it is what keeps the directory bounded without an ager or a TTL — an inbound attachment is re-downloaded per turn from the platform's file id, so a replayed turn is unaffected. The rest of `<state root>/channels/telegram/` is durable: git-ignored machine state that wants a volume on a long-running bot.
...
- `files/c-<chat>/` — downloaded inbound files, one directory per chat. **Scratch: emptied at every channel mount**, so it does NOT survive a restart (see above).
...
- **L1 (this channel).** An accepted turn's intent is persisted before the webhook ACK, then removed once its reply has been delivered. `start` has no graceful drain, so a crash **or** a rolling deploy exits mid-turn — but the intent survives and is replayed on the next start (the ACKed-but-un-completed window Telegram won't redeliver). This is **at-least-once**: replay re-runs the whole turn, so side-effecting tools re-run (safe for a Q&A bot; the bar for adding side-effecting ones) and, in a narrow pre-ACK crash window, a turn can run twice. A turn that keeps being interrupted mid-run is dropped after a few attempts and its asker is told to re-ask (this cannot distinguish a process-crashing turn from one repeatedly caught by deploys); if that turn had already recorded its answer, that answer is sent instead of the apology. An answer that was produced but never delivered (the process died between the model finishing and the message landing, or the send failed outright) is kept with the intent and re-delivered on the next start — without re-running the model or its tools. Where Telegram cannot prove whether the send landed, a duplicate is preferred to a loss. One residual window is deliberately not covered: if a state write fails outright (disk full), the turn is deferred to the next start rather than run untracked, which can replay it after later turns in the same session (a rare, disk-degraded reorder).
