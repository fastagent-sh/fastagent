...
**`railway --run`** drives the railway CLI to completion, mirroring `fly --run` (same credential carry via the neutral `assembleSecrets`, same runner seam faked in tests) but shaped by Railway's model (validated against CLI 5.15.0): auth needs an ACCOUNT credential (`railway login` / `RAILWAY_API_KEY`, not a project token — `init` predates any project); linkedness is read from `railway status --json` stdout (JSON = linked, empty = not; exit code is 0 either way), so an unlinked dir runs the full first deploy (`init` → `add --service` → `volume add`) while a linked one skips to redeploy (re-running `init`/`add` would DUPLICATE the project + service, splitting state); every command passes `--service` to stay non-interactive except `volume add` (no such flag — it rides the linked service, so the service is created first); secrets go one-per-`railway variables set --stdin` (value on stdin, never argv — Railway has no bulk stdin import); and the public domain is minted/read (`railway domain`) since a Railway service is unreachable until one exists.
