...
| `http.host` | Bind address for `dev` / `start`. Unset binds `127.0.0.1` (this machine only); `0.0.0.0` binds all interfaces. `--bind` overrides it, and is what you want for a one-off — this value travels into a deployed image, where `deploy` gates a non-wildcard value **if the Dockerfile does not pass `--bind` itself** (the generated one does, and the flag wins). See [Bind address](#bind-address). |
...
| `deploy.apt` | Extra apt packages baked into the generated image (`["git", "ripgrep"]` — Debian default repos). For a package needing a custom apt repo (e.g. `gh`) or a different base image, provide your own `Dockerfile` — `deploy` keeps an existing one (and warns that `deploy.apt` isn't applied to a hand-written Dockerfile). **A Dockerfile you own must bind the wildcard itself** (`CMD ["fastagent", "start", "/app", "--bind", "0.0.0.0"]`) — unless `http.host` already says `0.0.0.0`, which travels into the image and binds there. A serve binds `127.0.0.1` otherwise, and that container answers nothing from outside. `deploy` checks whichever one decides: for a Dockerfile it generated it looks for the flag; for one you wrote it looks for any mention of a wildcard rather than parsing your build, so a warning on a file that does bind it means the address is written somewhere this cannot see. `--run` is gated only when the finding is certain. A `Dockerfile` fastagent generated that later drifts from the current config (a changed `deploy.apt`, a new lockfile, a missing bind) is kept but flagged; `--force` regenerates it. |
...
loses capabilities when served is a different agent. What keeps that from being a hazard is exposure,
not capability — a serve binds loopback unless you say otherwise, so reaching those tools means
reaching the machine first (see [Bind address](#bind-address)).
