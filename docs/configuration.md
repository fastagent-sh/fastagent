...
| `deploy.apt` | Extra apt packages baked into the generated image (`["git", "ripgrep"]` — Debian default repos). For a package needing a custom apt repo (e.g. `gh`) or a different base image, provide your own `Dockerfile` — `deploy` keeps an existing one (and warns that `deploy.apt` isn't applied to a hand-written Dockerfile). **A Dockerfile you own must bind the wildcard itself** (`CMD ["fastagent", "start", "/app", "--bind", "0.0.0.0"]`): a serve binds `127.0.0.1` unless told otherwise, so without it the container answers nothing from outside. `deploy` warns when it finds no mention of the wildcard in your Dockerfile — it looks for the string rather than parsing your build, so a warning on a file that does bind it means the address is written somewhere this cannot see; a stale *generated* one gates `--run`. A `Dockerfile` fastagent generated that later drifts from the current config (a changed `deploy.apt`, a new lockfile, a missing bind) is kept but flagged; `--force` regenerates it. |
