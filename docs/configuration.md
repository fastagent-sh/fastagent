...
| `selfSchedule` | `false` | Mount the built-in `wake` tool so the agent can schedule its own follow-up turns (self-scheduling). Off by default — an autonomy capability, opt in when you want it; only active on the serving path (`dev`/`start` or `createAgentService`). Resident hosts poll locally; [AgentCore ingress](deploy.md#aws-bedrock-agentcore) uses external wake alarms. **What it does not bound:** an agent that writes `routines/<name>.ts` gives itself a cron whatever this says, armed within 30 seconds on a resident serve, with none of `wake`'s frequency floor. That is deliberate: with `bash` the agent can also write a crontab or loop `curl`, so no in-process switch would contain it; `wake`'s limits guard the model's mistakes inside a tool we own. What the routines path gives instead is visibility — every change is logged (`routines changed: + name (cron)`); see [#583](https://github.com/fastagent-sh/fastagent/issues/583). The bounds that hold are outside the agent's reach: a provider spend limit, and review before a release. |
