...
| `http.allowedHosts` | `localhost`, `127.0.0.1`, `[::1]` | Which names a caller may reach this serve BY — the `Host` header, not the origin — enforced only where the port is unpublished loopback (`dev`, or `start --bind 127.0.0.1` with no `--tunnel`). That is the one posture whose whole claim is "the port is the boundary", and a web page crosses it by rebinding its own hostname to `127.0.0.1`: the requests are then **same-origin**, so neither the cross-origin policy nor the JSON body gate applies. Set this when something else reaches the serve by name — a reverse proxy passing the original `Host` (cloudflared, Caddy and Traefik all do), or a `.local` name you dial yourself. Setting it REPLACES the default, like `http.cors`; an empty list is refused. A published port does not enforce it at all: a foreign `Host` is normal there and the port is already reachable without a browser. |
...
which is what stops a cross-origin write that skips the preflight. The one thing a bind address DOES buy you is
checked: an unpublished loopback serve refuses a request that names it anything but `localhost`/`127.0.0.1`/`[::1]`
(`http.allowedHosts`), which is what closes DNS rebinding — the trick that makes a page's requests look same-origin
and takes both of the above out of play.
