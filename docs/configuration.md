...
| `deploy.secrets` | Extra secret env-var names the deployed agent needs (e.g. `["GH_TOKEN"]`). `deploy` lists them in the runbook and, under `--run`, carries each value from your local env to the host secret store; a missing value gates the run. `fastagent info` and the deploy pre-flight scan `tools/`, `schedules/` and `channels/` for `process.env.NAME` reads and warn about names that are neither listed here, declared by a channel adapter, nor runtime-provided (`PORT`, `FASTAGENT_*`) — list a name here to silence a false positive. |
