...
| `http.cors` | follows the topology | Which origins a **browser** may call this serve from. Default: `*` when the port is published (a wildcard/LAN bind, or `--tunnel`) — this is an API, and anyone who can reach a published port can already curl it; loopback origins only when it is not (an unpublished `dev` serve is the one port whose only route in is your own browser). Setting this REPLACES the default in either direction: `["https://app.example.com"]` to let a front end call your dev serve, or to pin one origin instead of `*` on a deployment. `["*"]` cannot be combined with cookie credentials — a gateway doing cookie auth needs the exact origin. A channel's own route is never browser-callable whatever this says. |
...
A browser is the one caller with a default of its own, and it follows the bind: a published port is an
ordinary API (`*`), an unpublished loopback serve answers only pages on this machine — there your own
browser is the only route in, so handing every site you visit a working client is not a default worth
shipping. Either way our routes refuse a body that is not `application/json`, which is what stops a
cross-origin write that skips the preflight. `http.cors` replaces the default in either direction.
