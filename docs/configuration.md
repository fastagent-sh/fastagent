...
| `http.cors` | follows the bind | Which origins a **browser** may call this serve from. Default: `*` once the port is published — the bind is not loopback (`start`'s default), or `--tunnel` is on — because who may reach that network is your decision, not ours to guess at. `*` means any page your users visit can call this port from their browser and read the reply; on a public host that is nothing curl could not already do, on a LAN or VPC host it adds their network position. An unpublished loopback serve is the exception and allows only pages on this machine, since there your own browser is the only route in. Setting this REPLACES the default either way: `["https://app.example.com"]` to let a front end call your dev serve, or to pin one origin instead of `*` on a deployment. `["*"]` cannot be combined with cookie credentials — a gateway doing cookie auth needs the exact origin. A channel's own route is never browser-callable whatever this says. |
...
A browser is the one caller with a default of its own, and it follows the bind. Once the port is
published — a non-loopback bind, or `--tunnel` — the default is `*`: which network that port is on is
your decision, and we have less information about it than you do. Know what `*` grants: any page your
users visit can call the port from their browser and read the reply. On an unpublished loopback serve
the default is pages on this machine only, because there your own browser is the only route in.
Either way our routes refuse a body that is not `application/json`, which is what stops a cross-origin
write that skips the preflight. `http.cors` replaces the default in either direction, and
`--bind 127.0.0.1` takes the port off the network entirely.
