...
- `thread-participants.json` — a bounded record of what the Agent HEARD in each thread, written in every thread the channel can see, in every posture — including ones where the summon rule cannot read it. That follows from the referent anchor, which reads "has the Agent answered here?" in every chat type and under any route: that write is pinned, and the invariant that a thread the Agent answered in has heard every human who spoke there pins the other half with it. Slack's equivalent records less because it has no such second consumer: the humans it saw speak (capped at two, since the rule only asks whether a second one exists) and whether it has answered there. Nothing is read back from the platform, so losing the file costs one mention per thread to re-enter it,
