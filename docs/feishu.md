...
- `thread-participants.json` — a bounded record of what the Agent HEARD in each thread, written in every posture, including `@mentions`-only where the summon rule cannot read it. That is a deliberate position: Feishu learns its posture from a remote scope probe, and gating recording on it would put a correctness invariant (never under-count the humans in a thread) behind that probe's interpretation. Slack's equivalent records nothing in its restricted posture because it knows that posture from configuration, synchronously: the humans it saw speak (capped at two, since the rule only asks whether a second one exists) and whether it has answered there. Nothing is read back from the platform, so losing the file costs one mention per thread to re-enter it,
