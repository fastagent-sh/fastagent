...
- `thread-participants.json` — a bounded record of what the Agent HEARD in each thread, written in every posture (including `@mentions`-only, where the summon rule cannot read it — the scope posture is only known asynchronously, and under-recording a human is the one error that makes the Agent speak into a crowd; Slack's equivalent records nothing there, because it knows its posture from configuration): the humans it saw speak (capped at two, since the rule only asks whether a second one exists) and whether it has answered there. Nothing is read back from the platform, so losing the file costs one mention per thread to re-enter it,
