...
- `files/c-<chat>/` — downloaded inbound resources, one directory per chat. **Scratch: emptied at every channel mount** (every `start`, every `dev` restart), so unlike the files beside it, it does NOT survive a restart — a resource is re-downloaded per turn from its platform id, so a replayed turn is unaffected, but a path handed out by a previous process reads ENOENT.
