...
- **Pick the predicate with one answer.** File permissions under `.secrets/` cost four rounds of patches because the question being asked was "does this write carry a secret?" — which has a different answer at every call site and gains one more with every new writer. Each round fixed a real gap and created the next (`mode` is ignored once a file exists; `writeFileAtomic`'s own `mkdir` carries no mode; a resolved symlink lands in a third directory), and the mechanisms piling up (chmod an existing file, repair a directory, skip a symlink, warn about an older agent's mode) served scenarios with no reported use. The rule is now `SECRET_FILE_MODE` on a file fastagent CREATES, and nothing else: one question, one answer, and those scenarios stop existing rather than each getting a branch. When a fix keeps landing next to the last one, suspect the predicate before the call sites.
