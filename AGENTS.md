...
- **Pick the predicate with one answer.** When a fix keeps landing next to the last one, suspect the question before the call sites. Secrets-file permissions took four rounds while the question was "does this write carry a secret?" — answered differently at every call site, and gaining one more with each new writer. Asking "did fastagent create this file?" made the leftover cases stop existing instead of each earning a branch.
