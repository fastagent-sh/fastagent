...
├── state-lock.ts           # THE refusal: one writer per file-backed state (a listening socket, so the claim
│                           # ends exactly when its process does), over the RESOLVED write paths
